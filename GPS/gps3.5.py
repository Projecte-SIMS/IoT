...
def draw_radar(trail, sweep_angle, sats, speed):
    ...
    for r in range(RADAR_RADIUS):
        ...
        if 0 <= x < size and 0 <= y < size:
            grid[y][x] = Fore.CYAN + '|' + Style.RESET_ALL
    ...
    for dx, dy in trail:
        ...
        y = RADAR_RADIUS - dy
        if 0 <= x < size and 0 <= y < size:
            grid[y][x] = Fore.RED + 'O' + Style.RESET_ALL
    ...
    # Barra de satélites
    max_sats = 12
    sat_bar = Fore.YELLOW + '●' * min(sats, max_sats) + '○' * (max_sats - min(sats, max_sats)) + Style.RESET_ALL

    print(f"Satélites: {sat_bar}  Velocidad: {speed.get('speed_kmh','N/A'):.1f} km/h")
